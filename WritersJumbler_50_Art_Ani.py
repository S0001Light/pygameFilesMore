...
pygame.display.set_caption("Writers Jumbler 50 Art - start with keyboard letter q, then either w, e, r, t, y, u, i, o,  p=erase, b=blur, s=saves, animation=l,n,m")
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_b:
                autoBlur()
            if event.key == K_n:
                save_animation_gif1("WJ_marquee_s.gif", frames=50, rows=50, cols=50)
            if event.key == K_m:
                save_animation_gif2("WJ_marquee_e.gif", frames=50, rows=50, cols=50)
            if event.key == K_l:
                save_animation_gif3("WJ_marquee_se.gif", frames=101, rows=50, cols=50)
        screen.fill(bg_color)
        for sq in squares:
            pygame.draw.rect(screen, sq["color"], sq["rect"])
