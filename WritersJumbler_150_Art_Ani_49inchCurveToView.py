...
def surface_to_image(surface):
    ...
    return img
def save_animation_gif1(filename="WJ_S_Animation.gif", frames=50, rows=50, cols=150):
    gif_frames = []
...
def save_animation_gif2(filename="WJ_E_Animation.gif", frames=900, rows=50, cols=150):
    gif_frames = []
...
def save_animation_gif3(filename="WJ_SE_Animation.gif", frames=900, rows=50, cols=150):
    gif_frames = []
...
def save_animation_gif4(filename="WJ_S_Animation.gif", frames=900, rows=50, cols=150):
    gif_frames = []
...
def handle_hover_paint(pos):
    for sq in squares:
        if sq["rect"].collidepoint(pos):
            sq["color"] = fg_color
            break
def build_grid(cols=150, rows=50, start_x=405, start_y=20, cell_size=15, gap=5):
    global squares
    squares.clear()
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_b:
                autoBlur()
            if event.key == K_n:
                save_animation_gif1(zS+"_marquee_s.gif", frames=50, rows=50, cols=150)
            if event.key == K_m:
                save_animation_gif2(zS+"_marquee_e.gif", frames=50, rows=50, cols=150)
            if event.key == K_l:
                save_animation_gif3(zS+"_marquee_se.gif", frames=100, rows=50, cols=150) 
            if event.key == K_k:
                save_animation_gif4(zS+"_marquee_ne.gif", frames=100, rows=50, cols=150)      
        screen.fill(bg_color)
        for sq in squares:
            pygame.draw.rect(screen, sq["color"], sq["rect"])
