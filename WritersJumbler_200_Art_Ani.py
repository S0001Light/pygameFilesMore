...
pygame.display.set_caption("Writers Jumbler 200 Art - start with keyboard letter q, then either w, e, r, t, y, u, i, o,  p=erase, b=blur, s=saves, animation=l,k,n,m")
...
def surface_to_image(surface):
    ...
    return img
def save_animation_gif1(filename="WJ_S_Animation.gif", frames=200, rows=200, cols=200):
    gif_frames = []
...
def save_animation_gif2(filename="WJ_E_Animation.gif", frames=200, rows=200, cols=200):
    gif_frames = []
...
def save_animation_gif3(filename="WJ_SE_Animation.gif", frames=600, rows=200, cols=200):
    gif_frames = []
...
def save_animation_gif4(filename="WJ_S_Animation.gif", frames=600, rows=200, cols=200):
    gif_frames = []
...
def autoA30():
    # Detect grid size from squares[]
    rows = 0
    cols = 0

    # Find rows by scanning until x changes
    first_x = squares[0]["rect"].x
    for i, sq in enumerate(squares):
        if sq["rect"].x != first_x:
            rows = i
            break

    # Compute columns
    cols = len(squares) // rows

    # Paint one random square per column
    for col in range(cols):
...
class SliderGroup:
    def __init__(self, x, y, label, callback):
        ...
        green = (50, 255, 50)
        blue = (50, 50, 255)
        self.r = Slider(x, y,     300, 20, 0, color=red, callback=self.update)
        self.g = Slider(x, y+40,  300, 20, 0, color=green, callback=self.update)
        self.b = Slider(x, y+80,  300, 20, 0, color=blue, callback=self.update)
...
def handle_hover_paint(pos):
    for sq in squares:
        if sq["rect"].collidepoint(pos):
            sq["color"] = fg_color
            break
def build_grid(cols=200, rows=200, start_x=405, start_y=20, cell_size=3, gap=1):
    global squares
    squares.clear()
...
build_grid(cols=200, rows=200)

def saveAsPNG():
    word = "WritersJumbler_200_Art_"
    timestamp = datetime.datetime.now().strftime("%m-%d-%Y-%I-%M-%S-%p")

    png_filename = f"{word}{timestamp}.png"

    # Grab pixel data from the Pygame screen
    width, height = screen.get_size()
    pixel_data = pygame.image.tostring(screen, "RGB")

    # Convert raw pixel data → Pillow Image
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_b:
                autoBlur()
            if event.key == K_n:
                save_animation_gif1(zS+"_marquee_s.gif", frames=200, rows=200, cols=200)
            if event.key == K_m:
                save_animation_gif2(zS+"_marquee_e.gif", frames=200, rows=200, cols=200)
            if event.key == K_l:
                save_animation_gif3(zS+"_marquee_se.gif", frames=600, rows=200, cols=200) 
            if event.key == K_k:
                save_animation_gif4(zS+"_marquee_ne.gif", frames=600, rows=200, cols=200)      
        screen.fill(bg_color)
        for sq in squares:
            pygame.draw.rect(screen, sq["color"], sq["rect"])
