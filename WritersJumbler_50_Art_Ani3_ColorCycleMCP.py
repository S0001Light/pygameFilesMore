...
pygame.display.set_caption("Writers Jumbler 50 Art - start with keyboard letter q, then either w, e,  p=erase, b=blur, s=saves, animation=l,k,n,m")
...
def surface_to_image(surface):
    ...
    return img
def save_animation_gif1(filename="WJ_S_Animation.gif", frames=50, rows=50, cols=50):
    gif_frames = []
...
def save_animation_gif2(filename="WJ_E_Animation.gif", frames=50, rows=50, cols=50):
    gif_frames = []
...
def autoSymmetryPainterReverse():
    ...
    for row in range(rows):
        for col in range(cols // 2):
            ...
            # Now copy RIGHT → LEFT
            squares[left_index]["color"] = squares[right_index]["color"]
...
def handle_hover_paint(pos):
    for sq in squares:
        if sq["rect"].collidepoint(pos):
            sq["color"] = fg_color
            break
def build_grid(cols=50, rows=50, start_x=405, start_y=20, cell_size=15, gap=5):
    global squares
    squares.clear()
...
def generate_same_shade_color(base_color):
    ...
    # Keep same hue family but vary brightness slightly
...
def autoColorCycle():
    ...
    # Every 5 presses of Q → change FG color automatically
    if q_press_count % 7 == 0:
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_b:
                autoBlur()
            if event.key == K_n:
                save_animation_gif1(zS+"_marquee_s.gif", frames=50, rows=50, cols=50)
            if event.key == K_m:
