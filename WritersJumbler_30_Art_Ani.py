    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_b:
                autoBlur()
            if event.key == K_n:
                save_animation_gif1(zS+"_marquee_s.gif", frames=30, rows=30, cols=30)
            if event.key == K_m:
                save_animation_gif2(zS+"_marquee_e.gif", frames=30, rows=30, cols=30)
            if event.key == K_l:
                save_animation_gif3(zS+"_marquee_se.gif", frames=61, rows=30, cols=30) 
            if event.key == K_k:
                save_animation_gif4(zS+"_marquee_ne.gif", frames=61, rows=30, cols=30)
            screen.fill(bg_color)
        for sq in squares:
            pygame.draw.rect(screen, sq["color"], sq["rect"])
